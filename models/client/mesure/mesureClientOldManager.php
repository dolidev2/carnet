...
require_once 'models/client/mesure/mesureClientOldClass.php';

class mesureClientOldManager extends modelClass
{
    ...
    public function getMesuresMesure($mesures)
    {
        $mesures_data = [];
        if(!empty($this->mesures)){
            foreach ($this->mesures as $mesure){
                if($mesure->getMesure() === $mesures->getIdMesure()){
                    array_push($mesures_data,$mesure);
                }
            }
        }
    ...
    public function loadMesure()
    {
        ...
        foreach ($allMesure as $mesure)
        {
            ...
            $item = new mesureClientOldClass($data);
            $this->addMesure($item);
        }
    }
    ...
    {
        $query = "INSERT INTO mesureold(epaule_mesure,l_epaule_mesure,bas_mesure,poitrine_mesure,dos_mesure,t_taille_mesure,bassin_mesure,l_taille_mesure,
                   ...
                   e_p_poitrine_mesure,l_jupe_mesure,l_robe_mesure,l_poitrine_mesure,l_haut_mesure, creat_mesure,mod_mesure,sexe_mesure, t_tete_mesure, mesure)
                    VALUES (:epaule,:l_epaule,:bas,:poitrine,:dos,:t_taille,:bassin,:l_taille,:t_genou,:ceinture,:poignet,
                            ...
                            :l_chemise_a,:frappe, :carrure,:e_p_poitrine ,:l_jupe ,:l_robe ,:l_poitrine ,:l_haut ,:creat, :mod,:sexe, :t_tete,:mesure)";
        ...
        $stmt->bindValue(':mesure' , $data['mesure']);
        ...
        if($result >0 ){
            ...
            $mesure = new mesureClientOldClass($data);
    ...
    {
        $query = "DELETE FROM mesureold WHERE id_mesure=:id";
        $stmt = $this->getBdd()->prepare($query);
