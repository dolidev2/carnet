...
/**
 * The ImageLoader Utility is a framework to dynamically load image according to certain triggers,
 ...
 */
	...
	/**
	 * A group for image. A group can have one time limit and a series of triggers. Thus the image belonging to this group must share these constraints.
	 * @class ImgLoadGroup
	 * @extends Base
	 * @constructor
	 */
	...
	Y.ImgLoadGroup.ATTRS = {
		...
		/**
		 * Time limit, in seconds, after which image are fetched regardless of trigger events.
		 * @attribute timeLimit
		 * @type Number
		 */
		timeLimit: {
		...
		/**
		 * Distance below the fold for which image are loaded. Images are not loaded until they are at most this distance away from (or above) the fold.
		 * This check is performed at page load (domready) and after any window scroll or window resize event (until all image are loaded).
		...
		/**
		 * Class name that will identify image belonging to the group. This class name will be removed from each element in order to fetch image.
        ...
        /**
         * Determines how to act when className is used as the way to delay load image. The "default" action is to just
	...
	var groupProto = {
		...
		_init: function() {
			...
			/**
			 * Collection of image (<code>Y.ImgLoadImgObj</code> objects) registered with this group, keyed by DOM id.
			 * @property _imgObjs
			 * @private
			 * @type Object
			 */
			 ...
			 * During fold checks, if the user scrolls up then there's no need to check for newly exposed image.
		...
		addTrigger: function(obj, type) {
			 ...
			 *   groupA's trigger is removed and never fires, leaving image unfetched.
			 */
		 ...
		 * Initiates time limit for group; executes the fold check for the image.
		 * @method _onloadTasks
		 * @private
		 */
		...
		/**
		 * Displays the image in the group.
		 ...
		 */
		fetch: function() {
			Y.log('Fetching image in group: "' + this.get('name') + '".', 'info', 'imageloader');
		 ...
		 */
		_foldCheck: function() {
			Y.log('Checking for image above the fold in group: "' + this.get('name') + '"', 'info', 'imageloader');
			...
			if (allFetched) {
				Y.log('All image fetched; removing listeners for group: "' + this.get('name') + '"', 'info', 'imageloader');
				this._clearTriggers();
			}
		},
		...
		_fetchByClass: function() {
			...
			Y.log('Fetching all image with class "' + this._className + '" in group "' + this.get('name') + '".', 'info', 'imageloader');
	...
	Y.ImgLoadImgObj.ATTRS = {
		 ...
		 * Usually only used with src image.
		 * @attribute width
		 * @type Int
		 */
		width: {
		 ...
		 * Used when setting image as <code>visibility:hidden</code> prior to image fetching.
		 ...
		 * PNG image get special treatment in that the URL is specified through AlphaImageLoader for IE, versions 6 and earlier.
		 * Only used with background image.
		 * @attribute isPng
		 * @type Boolean
		 */
		isPng: {
	...
	var imgProto = {
		...
		_init: function() {
			 ...
			 * In the case of fold-conditional groups, image won't be fetched twice.
			 * @property _fetched
			 * @private
			 * @type Boolean
			 */
			 ...
			 * The Y position is checked only for image registered with fold-conditional groups. The position is checked first at page load (domready)
