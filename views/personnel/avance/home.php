                                ...
                                <div class="dt-responsive table-responsive">
                                    <table id="simpletable" class="table table-striped table-bordered nowrap">
                                        ...
                                        <?php if(!empty($avances)): ?>
                                            <?php $i=1; foreach ($avances as $av):?>
                                                <tr>
                                                    <td><?= $i ?></td>
                                                    <td><?= date("d-m-Y", strtotime($av->getCreatPaiePers())) ?></td>
                                                    <td><?= $av->getSommePaiePers() ?></td>
                                                    <td>
                                                        <?php if($_SESSION['agence_role']== 'Principale' && $_SESSION['role'] == 'admin'): ?>
                                                        <form class="d-inline" method="post" onsubmit="return confirm('Voulez-vous vraiment supprimer ?');" action="<?= URL ?>personnel/sava/<?= $av->getIdPaiePers() ?>" >
                        ...
                        <div class="modal fade" id="sign-in-modal" tabindex="-1">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <form action="<?= URL ?>personnel/ava/<?= $personnel->getIdPers() ?>" method="post">
                                            ...
                                            <div class="row form-group">
                                                <div class="col-sm-12">
                                                    <div>
                                                        ...
                                                        <input type="number" name="somme" class="form-control" placeholder="Entrez la somme" required>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
