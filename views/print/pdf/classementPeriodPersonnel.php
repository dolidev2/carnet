...
?>
    ...
    <div>
        <h1>
            Classement Personnel Agence  <?= $agence->getNomAgence()?> du <?= date("d/m/Y",strtotime($debut))  ?> au <?=date("d/m/Y",strtotime($fin)) ?>
        </h1>
    </div>
    <h3>Classement des collaborateurs</h3>
    <table>
        <thead>
        <tr>
            <th></th>
        ...
        if (isset($data) && !empty($data)):
            foreach ($data as $dt):
                ...
                <tr>
                    <td><?= $i ?></td>
                    <td class="desc"><?= $dt['personnel']['nom_pers']." ".$dt['personnel']['prenom_pers']." ".$dt['personnel']['contact_pers'] ?></td>
